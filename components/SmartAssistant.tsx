...
const SmartAssistant: React.FC = () => {
  ...
  const [messages, setMessages] = useState<{ role: 'user' | 'assistant', content: string }[]>([
    { role: 'assistant', content: "Hey! I'm Cuppy's digital assistant. Looking to book her for an event or just want to check the latest mixes?" }
  ]);
  ...
  const handleSend = async () => {
    ...
    try {
      ...
      const response = await ai.models.generateContent({
        ...
        contents: `You are an AI assistant for DJ Cuppy (Florence Otedola). 
        ...
        User said: ${userMsg}`,
        config: {
            systemInstruction: "Keep it brief and use emojis occasionally like 🎧 💖 ✨"
        }
      });

      setMessages(prev => [...prev, { role: 'assistant', content: response.text || "I'm having a bit of a glitch, but you can always reach Cuppy's team at bookings@djcuppy.com!" }]);
    } catch (error) {
      console.error(error);
      setMessages(prev => [...prev, { role: 'assistant', content: "Sorry, I'm currently off the decks. Try again in a bit!" }]);
    } finally {
      setIsTyping(false);
    }
  };
          ...
          >
            <div className="p-6 bg-pink-600 flex justify-between items-center shrink-0">
              <div className="flex items-center gap-3">
                ...
                <h3 className="font-bold tracking-widest text-sm uppercase">Cuppy Assistant</h3>
              </div>
              ...
              {messages.map((msg, i) => (
                <div key={i} className={`flex ${msg.role === 'user' ? 'justify-end' : 'justify-start'}`}>
                  <div className={`max-w-[80%] p-4 rounded-2xl text-sm ${
                    msg.role === 'user' ? 'bg-pink-600 text-white rounded-tr-none' : 'bg-white/5 border border-white/10 text-white/80 rounded-tl-none'
                  }`}>
                    {msg.content}
                  </div>
                </div>
